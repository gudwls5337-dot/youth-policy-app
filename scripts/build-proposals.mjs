...
const CAUSES = [
  ["유사사업 중복", /본질적으로\s*동일|중복성이\s*높|추진\s*중에\s*있으니|유사\s*(한\s*)?(사업|목적|형태)|중복\s*(지원|사업|투자|신청)|이미\s*(시행|운영|추진|하고)|기\s*시행|기시행|운영\s*(하고\s*)?있(음|으며)|추진\s*(하고\s*)?있(음|으며)|중앙정부\s*차원|정식\s*개통|구축\s*완료/],
  /* 「~성격이 강하다 → 해당 부서 검토가 선행」이 소관 이관의 실제 문구다(3-18·3-24).
     다만 「바람직할 것으로 사료」만으로는 소관 이관이 아니다 — 문화재단 출범 대기
     (3-5)가 그 문구를 쓰면서 실은 시기상조였다(2026-07-31 감사). */
  ["소관 아님", /소관\s*(부서|기관|사항)|우리\s*(과|시|부서).{0,10}(소관|업무).{0,6}(아님|아니)|타\s*부서|도\s*조례\s*소관|국가\s*사무|성격이\s*강한|담당\s*부서의?\s*검토가\s*선행/],
  ["법령 위배·근거 부족", /조례\s*개정\s*후|법적\s*근거|근거\s*(가\s*)?(없|부족|미비)|조례\s*(개정|제정)\s*필요|해당된다\s*보기\s*어려|상충|위배|「[^」]*법」\s*제\d/],
  ["예산 부담", /과다하여|예산\s*대비\s*효율성이\s*낮|예산.{0,14}(소요|부담|과다|확보\s*어려|편성\s*곤란)|재정\s*부담|지속적인\s*재원|많은\s*예산/],
  ["실행 곤란", /현실(성|적).{0,12}(없|어려|곤란)|구성.{0,10}어려움|집행.{0,8}어려|증빙.{0,8}어려|관리.{0,8}어려|피상적/],
  ["효과 불확실·수요 불명", /이어지기\s*어렵|수요\s*(가\s*)?(없|불명|파악|조사)|구인\s*신청\s*건수\s*0|실효성|효과.{0,10}(제한적|미미|없|낮|불확실)|유명무실|한계가\s*있/],
  ["민원·갈등 우려", /민원|분쟁|찬성측과\s*반대측|불만이\s*있을\s*수|갈등/],
  ["형평성", /형평성|특정\s*(연령|계층|대상|집단).{0,24}(제한|한정|특정하여)|역차별/],
  ["시기상조", /장기적인?\s*관점|당장에?\s*추진하기\s*보다|초기\s*단계|출범하였으나|장기\s*검토|추후\s*검토|재검토|안정화\s*(이후|후)|여건\s*조성|우선\s*확인\s*후/],
];
